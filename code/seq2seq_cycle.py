...
class Model(object):
  def __init__(self, args, vocab):
    ...
    # VAE part - now disabled!
    self.kld_loss = self.C
...
if __name__ == '__main__':
  ...
  with tf.Session(config=config) as sess:
    ...
    if args.mass_online_testing:
      ...
      form_path = os.path.join(args.model,
                               'baseline_{}_{}_form.txt'.format(args.anneal,
                                                                args.C))
      inform_path = os.path.join(args.model,
                                 'baseline_{}_{}_inform.txt'.format(args.anneal,
                                                                    args.C))
      online_transfer(neg_lines,
                      write_file=form_path,
                      style=0)
      online_transfer(pos_lines,
                      write_file=inform_path,
                      style=1)
    ...
    if args.latent_testing:
      ...
      # val and test are the same here!!!
      # val_batches, _, _ = get_batches(val0, val1, vocab.word2id,
      ...
      for batch in test_batches:
        c += 1
        if c % 100 == 0: print('{} / {}'.format(c, len(test_batches)))
